...
def _listdir(path):
    return [sub_path for sub_path in os.listdir(path) if not sub_path.startswith('@')]


class MVTecDataLoader(object):
    def __init__(self, root_dir, shot_num, way_num, episode_test_sample_num, shuffle_images=False):
        ...
        metatrain_folder = os.path.join(root_dir, 'train')
        metatest_folder = os.path.join(root_dir, 'test')

        npy_dir = f'./mvtec_{os.path.basename(root_dir)}_episode_filename_list/'
    ...
    def get_images(self, paths, labels, nb_samples=None, shuffle=True):
        ...
        images = [(i, os.path.join(path, image)) \
            ...
            for image in sampler(_listdir(path))]
        if shuffle:
    ...
    def process_batch(self, input_filename_list, input_label_list, batch_sample_num, reshape_with_one=True):
        ...
        for filepath in new_path_list:
            ...
            # TODO pass image size
            image = image.resize((256, 256), Image.LANCZOS)
            image = np.asarray(image).astype(np.float32) / 255.0
            img_list.append(image)
            label_names.append(Path(filepath).parent.name)
        ...
        return img_array, label_array, label_names
    ...
    def get_batch(self, phase='train', idx=0):
        ...
        this_inputa, this_labela, this_label_namea = self.process_batch(this_task_tr_filenames, this_task_tr_labels,
                                                                        epitr_sample_num, reshape_with_one=False)
        this_inputb, this_labelb, this_label_nameb = self.process_batch(this_task_te_filenames, this_task_te_labels,
                                                                        epite_sample_num, reshape_with_one=False)

        return this_inputa, this_labela, this_label_namea, this_inputb, this_labelb, this_label_nameb
